...
markerline, stemlines, baseline = plt.stem(xrange, func(xrange, 1))  # Placeholder values
...
with writer.saving(fig, 'sinWaveDiscreet.mp4', 200):
    ...
    for A in np.arange(0.5, 1.5+stepsize_A, stepsize_A):
        ...
        at.txt.set_text(f'A = {"{:.1f}".format(A)}')
        plt.draw()
        ...
        stemlines.remove()
        baseline.remove()
        markerline, stemlines, baseline = plt.stem(xrange, ydata)
